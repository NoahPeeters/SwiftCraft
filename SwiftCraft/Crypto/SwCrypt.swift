...
open class PEM {
    ...
    open class EncryptedPrivateKey {
        ...
        open static func toDER(_ pemKey: String, passphrase: String) throws -> Data {
            ...
            let base64Data = strippedKey[strippedKey.index(strippedKey.startIndex, offsetBy:aesHeaderLength)...]
            guard let data = PEM.base64Decode(String(base64Data)) else {
                throw SwError(.parse("base64decode"))
            }
        ...
        fileprivate static func getIV(_ strippedKey: String) -> Data? {
            let lowerIndex = strippedKey.index(strippedKey.startIndex, offsetBy:aesInfoLength)
            let upperIndex = strippedKey.index(strippedKey.startIndex, offsetBy:aesHeaderLength)
            
            let ivInHex = strippedKey[lowerIndex..<upperIndex]
            return String(ivInHex).dataFromHexadecimalString()
        }
...
extension String {
    ...
    public func dataFromHexadecimalString() -> Data? {
        ...
        while let i = index {
            let byteString = trimmedString[i..<trimmedString.index(i, offsetBy: 2)]
